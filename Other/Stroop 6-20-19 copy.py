...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/jordanfuller/Desktop/Experiment Building/4. Stroop/Stroop 6-20-19 copy.py',
...
ISIClock = core.Clock()
ISI_wait = clock.StaticPeriod(win=win, screenHz=expInfo['frameRate'], name='ISI_wait')
...
for thisPractice in Practice:
    ...
    # keep track of which components have finished
    TrialComponents = [Stroop_Text, key_resp_3, practice_cue_txt, prac_cont]
    ...
    while continueRoutine:
        ...
        # *prac_cont* updates
        if t >= 1.0 and prac_cont.status == NOT_STARTED:
            # keep track of start time/frame for later
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        # update/draw components on each frame
        # *ISI_wait* period
        if t >= 0.0 and ISI_wait.status == NOT_STARTED:
            # keep track of start time/frame for later
            ISI_wait.tStart = t  # not accounting for scr refresh
            ISI_wait.frameNStart = frameN  # exact frame index
            win.timeOnFlip(ISI_wait, 'tStartRefresh')  # time at next scr refresh
            ISI_wait.start(1)
        elif ISI_wait.status == STARTED:  # one frame should pass before updating params and completing
            ISI_wait.complete()  # finish the static period
